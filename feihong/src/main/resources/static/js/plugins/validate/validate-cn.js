...
$.validator.setDefaults({
    ...
    success: function (span) {
      if(span.prev().length!=0){
        if (span.prev().is(":radio") || span.prev().is(":checkbox") || $(span).prev().prop("tagName").toLowerCase() == "select") {
            $(span).addClass("hide")
        }else if (span.prev().hasClass("chosen-container")){
            span.prev().removeClass("error");
            $(span).addClass("success fa fa-check select-success");
        }
        else {
    ...
    highlight: function (element, errorClass) {
       ...
       console.log($(element).parent().next())
        ...
        $(element).parent().next().removeClass("hide success fa fa-check");
    },
