(function($) {
    ...
    function updateSelectedAndOnChange($selectionContainer, $selectedContainer, $originalSelect, options) {
        ...
        function updateOriginalSelect() {
            ...
            if($selectedContainer.find('.title')){
                $selectedContainer.find('.title').remove();
            }

            $selectedContainer.prepend("<div class='title'>已选择组员（"+selected.length+"人）</div>");
    ...
    UiBuilder.prototype.build = function(el, hideSidePanel) {
        ...
        $(selected).prepend("<div class='title'>已选择组员（0人）</div>");
